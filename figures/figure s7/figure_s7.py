...
def figure_1():
    ...
    # OS Cwd
    path = os.getcwd()+"\\figures\\figure s7\\"
    ...
    for i, angle in enumerate(angle_list):
        KN = experiment("KN", str(int(i)), 1, 25, 35, 4, StressStrain=True, Width=75, Height=150, Thickness=1, Verbose=False, path=parentfolder+"\\data\\uniaxial_planar\\KN-{}\\Specimen_RawData_1.csv".format(angle))
        strain = KN.strain
        stress = KN.stress
    ...
    SS_df = pd.DataFrame({'Displacement': KN.disp, 'Force': KN.force})
    SS_df.to_csv(path+'KN-exp.csv', index=False)
    ...
    # FEM Data
    for i, angle in enumerate(angle_list):
        df = fig.load_csv(os.getcwd()+'\\data\\FEM_planar\\OGD_knit\\Abaqus_Uniaxial_Sim.csv')
        strain = (df['Displacement']/150)
        stress = df['Force']/(75)
    ...
    # Saving of figure
    fig.savefig(path + 'figure_s7.pdf', dpi=800)
